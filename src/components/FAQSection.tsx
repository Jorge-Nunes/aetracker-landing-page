  ...
  return <section id="faq" className="py-20 bg-background">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="max-w-4xl mx-auto space-y-4">
          {faqs.map((faq, index) => <div key={index} className="bg-card rounded-lg border border-brand-red/20 hover:border-brand-red/40 transition-all duration-300">
              <button className="w-full p-6 text-left flex justify-between items-center" onClick={() => toggleFAQ(index)}>
                ...
                {openIndex === index ? <ChevronUp className="w-5 h-5 text-brand-red flex-shrink-0" /> : <ChevronDown className="w-5 h-5 text-brand-red flex-shrink-0" />}
              </button>
              
              {openIndex === index && <div className="px-6 pb-6 animate-fade-in">
        ...
        <div className="text-center mt-12">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="https://wa.me/5511999999999" target="_blank" rel="noopener noreferrer" className="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-lg font-semibold transition-colors inline-flex items-center justify-center">📱 WhatsApp: (11) 98773-3033</a>
            <a href="tel:+551140004000" className="bg-brand-red hover:bg-brand-red-hover text-brand-white px-6 py-3 rounded-lg font-semibold transition-colors inline-flex items-center justify-center">📞 Central: (11) 94345-3069</a>
          </div>
        </div>
      </div>
    </section>;
